...
class Generator_1D(nn.Module):
    ...
    def forward(self, x):
        ...
        layer1 = self.layer1(x)
        if self.test: print ('layer1 G:',layer1.shape)
        layer2 = self.layer2(layer1)
    ...
    for batch_idx,  real in enumerate(loader):
        ...
        if batch_idx % 100 == 0:
            ...
            with torch.no_grad():
                ...
                for i in range(32):
                    end.append(ssim(np.squeeze(np.moveaxis(real[i].detach().cpu().numpy(), 0, 1)),np.squeeze(np.moveaxis(fake[i].detach().cpu().numpy(), 0, 1)),multichannel=True))
            ...
            step += 1
#%%
print (torch.randn(20, 16, 50).shape)
