...
class Discriminator_1D(nn.Module):
    def __init__(self, channels_img, features_d):
        ...
        self.disc = nn.Sequential(
            ...
            nn.Conv1d(
                channels_img, features_d, kernel_size=4, stride=1, padding=0
            ),
            nn.LeakyReLU(0.2),
            # _block(in_channels, out_channels, kernel_size, stride, padding)
            self._block(features_d, 140, 4, 1, 0),
            self._block(140, 100 , 4, 1, 0),
            self._block(100 , 80 , 4, 1, 0),
            # After all _block img output is 4x4 (Conv2d below makes into 1x1)
            nn.Conv1d(80, 1, kernel_size=4, stride=1, padding=0),
            nn.Sigmoid(),
        )
